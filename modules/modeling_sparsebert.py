...
def round_to_multiple_of_eight(size):
    return round(size * 1.0 / 8) * 8
...
class SparseBertLayer(nn.Module):
    ...
    def forward(
        ...
        # This manner is somehow fine for checkpoint loading, sparsification, and densification ; ).
        ...
        layer_output = apply_chunking_to_forward(
            self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attention_output, neuron_mask
        )
        outputs = (layer_output,) + self_outputs[1:]
        return outputs

    def feed_forward_chunk(self, attention_output, neuron_mask):
        intermediate_output = self.intermediate(attention_output)
        # Mask neurons if we want to.
        if neuron_mask is not None:
            assert intermediate_output is not None, "neuron mask should be applied without skipping."
